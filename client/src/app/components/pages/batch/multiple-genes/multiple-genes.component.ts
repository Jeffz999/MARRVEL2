...
export class MultipleGenesComponent implements OnInit {
	...
	fetchData(from, to) {
		...
		this.loading = true;
		this.api
			.getGeneBatchByArray(this.genesToDisplay)
			.pipe(take(1))
			.subscribe(
				(res) => {
					this.data = res;
			...
			if (page === this.curPage && !this.loading) {
				...
				tasks.push(
					new Observable((observer) => {
						this.api.getGeneBatchByArray(this.genes.slice(gFrom, gTo)).subscribe((res) => {
							this.wholeGenesHaveData += gTo - gFrom;
							observer.next(res);
						});
					}).pipe(first()),
	...
	getBlobUrl(fileType, dataToDownload) {
		...
		switch (fileType) {
			case '.tsv':
				dataString =
					'symbol\tmim_number\tomim_phenotype\tomim_allele\t' +
