...
export class BatchComponent implements OnInit {
    ...
    fetchData(vFrom, vTo) {
        ...
        this.tsvPageDownloadUrl = null;
        this.api
            .getBatchByArray(this.variants.slice(vFrom, vTo))
    ...
    processDataRow(row: VariantResult) {
        if (row.gnomADVar) {
            row.gnomADVar.alleleCount =
                ((row.gnomADVar.genome || {}).alleleCount || 0) +
                ((row.gnomADVar.exome || {}).alleleCount || 0);
            row.gnomADVar.alleleNum =
                ((row.gnomADVar.genome || {}).alleleNum || 0) +
                ((row.gnomADVar.exome || {}).alleleNum || 0);
            row.gnomADVar.alleleFreq = row.gnomADVar.alleleCount / row.gnomADVar.alleleNum;
            row.gnomADVar.homCount =
                ((row.gnomADVar.genome || {}).homCount || 0) +
                ((row.gnomADVar.exome || {}).homCount || 0);
        }
        if (row.dgv) {
            row['dgvVarLoss'] = 0;
