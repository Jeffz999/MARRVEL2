...
export class BatchComponent implements OnInit {
	...
	fetchData(vFrom, vTo) {
		...
		this.loading = true;
		this.tsvPageDownloadUrl = null;
		this.api.getBatchByArray(this.variants.slice(vFrom, vTo)).subscribe((res: VariantResult[]) => {
	...
	processDataRow(row: VariantResult) {
		if (row.gnomADVar) {
			row.gnomADVar.alleleCount =
				((row.gnomADVar.genome || {}).alleleCount || 0) + ((row.gnomADVar.exome || {}).alleleCount || 0);
			row.gnomADVar.alleleNum =
				((row.gnomADVar.genome || {}).alleleNum || 0) + ((row.gnomADVar.exome || {}).alleleNum || 0);
			row.gnomADVar.alleleFreq = row.gnomADVar.alleleCount / row.gnomADVar.alleleNum;
			row.gnomADVar.homCount =
				((row.gnomADVar.genome || {}).homCount || 0) + ((row.gnomADVar.exome || {}).homCount || 0);
		}
		if (row.dgv) {
			row['dgvVarLoss'] = 0;
	...
	createDownloadUrl(fileType: string, whole?: boolean) {
		if (!whole) {
			...
			for (let page = 0; page < nPage; ++page) {
				...
				if (page === this.curPage && !this.loading) {
					// not fetching data again for current page
					tasks.push(of(this.dataSource.data));
					this.wholeVarsHaveData += vTo - vFrom;
				} else {
					tasks.push(
						new Observable((observer) => {
							this.api.getBatchByArray(this.variants.slice(vFrom, vTo)).subscribe((res) => {
								this.wholeVarsHaveData += vTo - vFrom;
								observer.next(res);
							});
						}).pipe(first()),
	...
	getBlobUrl(fileType, dataToDownload) {
		...
		switch (fileType) {
			case '.tsv':
				dataString =
					'variant\tgnomAD_AC\tgnomAD_AN\tgnomAD_AF\tgnomAD_hom\t' +
