...
export class PhenotypesComponent implements OnInit {
    ...
    ngOnInit() {
        if (this.gene) {
            this.phenotypes[9606] = this.phenotypes[9606] || [
                { gene: this.gene, bestScore: true, phenotypes: null },
            ];
            if (this.gene.phenotypes && this.gene.phenotypes.length && this.gene.phenotypes[0].id) {
                ...
                for (const phenotype of this.gene.phenotypes) {
                    if (
                        phenotype.ontology &&
                        phenotype.ontology.categories &&
                        phenotype.ontology.categories.length
                    ) {
        ...
        if (this.orthologs && this.orthologs.length) {
            for (const ortholog of this.orthologs) {
                ...
                const phenoCats = [];
                for (const phenotypes of [
                    ortholog.gene2.phenotypes,
                    ortholog.gene2.impcPhenotypes,
                ]) {
                    ...
                    for (const phenotype of phenotypes) {
                        ...
                        if (
                            ...
                            for (const cat of phenotype.ontology.categories) {
                                ...
                                relExists = true;
                                if (
                                    aGenePheno[catName].filter(
                                        (pheno) => pheno.id === phenotype.ontology.id,
                                    ).length === 0
                                ) {
                ...
                this.phenotypes[taxonId].push({
                    ...
                    impcPhenotypes:
                        ortholog.gene2.impcPhenotypes && ortholog.gene2.impcPhenotypes.length
                            ? phenoCats[1]
                            : null,
    ...
    getTermDetailUrl(taxonId: number, term: any, isGene: boolean) {
        ...
        case 9606:
            return isGene
                ? `https://hpo.jax.org/app/browse/gene/${term.entrezId}`
                : `https://hpo.jax.org/app/browse/term/${term}`;
        case 10090:
            return isGene
                ? `http://www.informatics.jax.org/marker/${term.mgiId}`
                : `http://www.informatics.jax.org/vocab/mp_ontology/${term}`;
        case 10116:
            return isGene
                ? `https://rgd.mcw.edu/rgdweb/report/gene/main.html?id=${term.rgdId}`
                : `http://www.informatics.jax.org/vocab/mp_ontology/${term}`;
        case 6239:
            return isGene
                ? `https://wormbase.org/species/c_elegans/gene/${term.wormBaseId}`
                : `https://www.wormbase.org/species/all/phenotype/${term}`;
        case 7227:
            return isGene
                ? `https://flybase.org/reports/${term.flyBaseId}`
