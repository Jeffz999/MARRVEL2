		...
		if (orthologs && orthologs.length) {
			for (const ortholog of orthologs) {
					...
					for (const phenotype of phenotypes) {
						...
						if (
							...
							for (const cat of phenotype.ontology.categories) {
								...
								relExists = true;
								if (
									aGenePheno[catName].filter((pheno) => pheno.id === phenotype.ontology.id).length ===
									0
								) {
				...
				this.phenotypes[taxonId].push({
					...
					impcPhenotypes:
						ortholog.gene2.impcPhenotypes && ortholog.gene2.impcPhenotypes.length ? phenoCats[1] : null,
