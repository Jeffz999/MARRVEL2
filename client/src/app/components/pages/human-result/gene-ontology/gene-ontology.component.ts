...
export class GeneOntologyComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
            ...
            if (this.gene.gos) {
                for (const go of this.gene.gos) {
                    if (go.eviCode in EXP_EVICODES && go.ontology) {
                        const slimGoId =
                            go.ontology.agrSlimGoId || NAMESPACE_TO_GOID[go.ontology.namespace];
                        if (slimGoId) {
                            this.gos['human']['gos'][slimGoId] =
                                this.gos['human']['gos'][slimGoId] || [];
        ...
        if (this.orthologs && this.orthologs.length) {
            ...
            for (const ortholog of this.orthologs) {
                ...
                if (!ortholog.gene2 || !ortholog.gene2.gos || !ortholog.gene2.gos.length) {
                    continue;
                }
                this.visHeight += 20;
    ...
    selectGeneGo(orgName, idx, goId) {
        if (
            this.selected &&
            this.selected['orgName'] === orgName &&
            this.selected['idx'] === idx &&
            this.selected['goId'] === goId
        ) {
            this.selected = null;
            return;
        }
        ...
        const gos =
            orgName === 'human' ? this.gos[orgName].gos[goId] : this.gos[orgName][idx].gos[goId];
