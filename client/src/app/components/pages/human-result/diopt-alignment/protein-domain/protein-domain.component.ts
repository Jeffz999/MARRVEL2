    ...
    initTableAcc() {
        ...
        this.dataSource.sortData = (data: DomainData[], sort: MatSort) => {
            return data.sort((a: DomainData, b: DomainData) => {
                ...
                switch (sort.active) {
                case 'domainStart':
                    const aStart = isNaN(+a.domainStart)
                        ? +a.domainStart.substr(1)
                        : +a.domainStart;
                    const bStart = isNaN(+b.domainStart)
                        ? +b.domainStart.substr(1)
                        : +b.domainStart;
                    return (aStart < bStart ? -1 : 1) * dirMul;
                case 'domainStop':
                    const aStop = isNaN(+a.domainStop)
                        ? +a.domainStop.substr(1)
                        : +a.domainStop;
                    const bStop = isNaN(+b.domainStop)
                        ? +b.domainStop.substr(1)
                        : +b.domainStop;
