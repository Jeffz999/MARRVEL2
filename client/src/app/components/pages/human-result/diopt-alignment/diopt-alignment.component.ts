...
export class DioptAlignmentComponent implements OnInit {
                    ...
                    (res) => {
                        ...
                        if (res && res.data && res.data.length) {
                            ...
                            for (const row of res.data) {
                                ...
                                row.display =
                                    row.species === 'mark' ||
                                    row.species in this.speciesToHighlight;
                            }
        ...
        for (const row of this.data) {
            ...
            if (row.species !== 'mark') {
                ...
                for (let idx = 0; idx < row.proteins.length; ++idx) {
                    if (
                        this.speciesToHighlight[row.species] &&
                        this.highlightFrom <= row.realIdx[idx] &&
                        row.realIdx[idx] <= this.highlightTo
                    ) {
    ...
    scrollToAlignment() {
        window.document
            .getElementById('alignment-highlight')
