  ...
  if (!this.cy.getElementById(this.gene().symbol).isNode()) {
		...
		for (const edge of this.data()) {
			...
			for (const interactorEdge of edge.interactor.ppis) {
				...
				if (
					interactorEdge.evidences?.length >= this.eviThreshold &&
	...
	addEdge(edge): void {
		...
		this.getNodeByIdOrAdd(edge.interactor.symbol, {
			id: edge.interactor.symbol,
			name: edge.interactor.symbol,
		});
		...
		return this.cy.add([
			{
				...
				style: {
					...EDGE_CONFIG,
					opacity: edgeData['weight']
						? edgeData['weight'] / this.evidenceRange[this.evidenceRange.length - 1]
						: 1,
				},
				data: edgeData,
			},
