...
export class SearchBoxComponent implements OnInit {
    ...
    onGeneInput(e: KeyboardEvent, value: string): void {
        ...
        const keyword = value;
        if (keyword) {
            this.api.getGenesBySymbolPrefix(9606, keyword).subscribe((res) => {
    ...
    getResult(): void {
        ...
        switch (this.selectedInputType) {
            ...
            case "protein":
                ...
                break;
            default:
                this.searchSvc.redirect(this.selectedInputType as "gene" | "protein", this.gene, this.variant, this.genomeBuild);
                break;
        }
    }

    search(keyword?: string): void {
        this.router.navigate(["search", "human"], {
            queryParams: {
    ...
    openYouTubeDialog() {
        const dialogRef = this.dialog.open(YoutubeDialogComponent, {
            width: "608px",
            data: {
                url: this.sanitizer.bypassSecurityTrustResourceUrl("https://www.youtube.com/embed/e1Qy3HC3gLo"),
            },
        });
    }
}
