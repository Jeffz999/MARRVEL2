...
const COMPARE = (a: any, b: any, aSmall: number, bSmall: number, abTie: number | null, superiorValue?: any) => {
	if (a === b) {
		return abTie;
	}
...
export class ModelmatcherService {
	...
	getScientistsByGeneSymbol(geneSymbol: string): Observable<unknown> {
		return new Observable((obs) => {
			this.http
				.get(
					`${environment.mmApiHost}/SearchScientistByGene` +
				...
				.subscribe({
					next: (res) => {
						obs.next(
							(res['ScientistMatches'] || [])
								.sort((a, b) => {
									const cmpTaxonId = COMPARE(
										this.taxonIdToOrder[a.modelOrganism.id],
