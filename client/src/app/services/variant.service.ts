...
export class VariantService {
	...
	liftoverHg38ToHg19(variant: Variant): Observable<LiftoverResponse> {
		const url = `${environment.apiHost}/data/liftover/` + `hg38/chr/${variant.chr}` + `/pos/${variant.pos}/hg19`;
	...
	liftoverHg19ToHg38(variant: Variant): Observable<LiftoverResponse> {
		const url = `${environment.apiHost}/data/liftover/` + `hg19/chr/${variant.chr}` + `/pos/${variant.pos}/hg38`;
		return new Observable((obs) => {
			this.http
				.get(url)
				.pipe(take(1))
