...
export class ApiService {
        ...
        return new Observable((observer) => {
            this.http.get(url).subscribe(
                (res: any) => {
                    ...
                    res.exome.alleleFreq = res.exome.alleleNum
                        ? res.exome.alleleCount / res.exome.alleleNum
                        : undefined;
                    res.genome.alleleFreq = res.genome.alleleNum
                        ? res.genome.alleleCount / res.genome.alleleNum
                        : undefined;
    ...
    getBatchByArray(data): Observable<any> {
        ...
        return new Observable((observer) => {
            this.http.get(url, { params: { variants: data } }).subscribe(
                (res) => {
                    observer.next(res);
                },
                (err) => {
    ...
    getGeneBatchByArray(data): Observable<any> {
        ...
        return new Observable((observer) => {
            this.http.get(url, { params: { entrezIds: data.map((d) => d.entrezId) } }).subscribe(
                (res) => {
                    observer.next(res);
                },
                (err) => {
    ...
    getSmartDomain(gene: HumanGene): Observable<any> {
        return new Observable((observer) => {
            this.http
                .get(
                    `${environment.apiHost}/data//gene/entrezId/${gene.entrezId}/protein/domain/smart`,
                )
                .subscribe({
                    next: (res) => {
                        observer.next(res);
    ...
    getPPI(gene: HumanGene): Observable<any> {
        return new Observable((observer) => {
            this.http
                .get(`${environment.apiHost}/data/ppi/entrezId/${gene.entrezId}`)
                .pipe(take(1))
                .subscribe({
                    next: (res) => {
                        observer.next(res);
