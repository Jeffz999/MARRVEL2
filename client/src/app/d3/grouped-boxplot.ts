...
export class GroupedBoxplot {
    ...
    constructor(data: Point[], options?: BoxConfig) {
        ...
        for (const G of this.groupedData) {
            this.initGroupVisual(
                G,
                svg,
                this.yScale,
                this.groupColorScale(G.name),
                this.tooltipDiv,
            );
        }
    }
    ...
    initColorScale(groupValues) {
        ...
        for (
            let i = (groupValues.length / 2) % 2 !== 0 ? 0 : 1;
            i < groupValues.length / 2;
            i += 2
        ) {
            const idx2 = groupValues.length / 2 + i;
            groupValues[idx2] = [groupValues[i], (groupValues[i] = groupValues[idx2])][0];
        }
        // Set range
        const colorRange = groupValues.map((v, i, arr) =>
            d3.interpolateSpectral(i / (arr.length - 1)),
        );
    ...
    initXScale(groups, totalBoxes) {
        ...
        for (const G of groups) {
            for (const box of G.boxes) {
                ...
                const q2 = d3.quantile(box.y, 0.5);
    ...
    initGroupVisual(group, svg, y, groupColor, div) {
        ...
        g.append('path')
            ...
            .on('mouseover', (d) => {
                ...
                div.html(
                    `<strong>Organ</strong>: ${group.name}<br>
        ...
        const xAxisLabelLimit = Math.ceil(
            (((this.options.margin.b - 12) * 2) / xAxisFontSize) * 1.25,
        );
        const xAxis = (gr) =>
            gr.attr('transform', `translate(0,${this.height - this.options.margin.b})`).call(
                d3
                    .axisBottom(x)
                    .ticks(group.boxes.map((b) => b.x))
                    .tickSizeOuter(0),
            );
        svg.append('g')
            .attr('class', 'xaxis')
            ...
            .text((text) => {
                return text.length < xAxisLabelLimit
                    ? text
                    : text.substr(0, xAxisLabelLimit) + ' ...';
            })
            .on('mouseover', (text) => {
                div.transition().duration(200).style('opacity', 0.9);
                div.html(text)
