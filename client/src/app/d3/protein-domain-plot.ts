...
export class ProteinDomainPlot {
	...
	setViewScale(ratio?: number) {
		...
		// Set scale
		this.scale = d3.scaleLinear().domain([0, posMax]).range([this.options.margin.l, this.viewWMax]);
	}
			...
			g
				...
				.call(
					d3
						...
						.ticks(Math.ceil((this.scale.domain()[1] - this.scale.domain()[0]) / 100) + 1),
				)
	...
	drawRectDom(dom: ProteinDomain, rx?: number, ry?: number) {
		return (g) => {
			const gr = g.append('g').attr('transform', `translate(${this.scale(dom.start)},${this.options.margin.t})`);
			gr.append('rect')
				.attr('width', this.scale(dom.end) - this.scale(dom.start))
