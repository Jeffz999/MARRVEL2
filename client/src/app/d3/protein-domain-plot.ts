...
export class ProteinDomainPlot {
    ...
    public xScale(ratio?: number): number {
        ...
        if (this.viewWMax <= this.options.width) {
            // If the plot length got shorter than view width, reset dragged offset
            this.wrapperDX = 0;
        } else {
            // Don't go further than right end
            this.wrapperDX = Math.max(
                this.wrapperDX,
                -this.viewWMax + this.options.width - this.options.margin.r,
            );
        }
        this.draw();
        return this.xViewScale;
    ...
    setViewScale(ratio?: number) {
        ...
        // Calc view max
        const posMax =
            Math.ceil(d3.max(this.data.map((d) => (d.end != null ? d.end : d.start))) / 100) * 100; // Round from 100
    ...
    draw() {
        ...
        if (this.viewWMax > this.options.width) {
            // Add drag behavior
            wrapper.call(
                d3.drag().on('drag', (data, idx, group) => {
                    this.wrapperDX = Math.max(
        ...
        const axis = (g) =>
            g
                .attr(
                    'transform',
                    `translate(${this.options.margin.l},${this.options.height - this.options.margin.b})`,
                )
                .attr('class', 'axis')
                .call(
                    d3
                        ...
                        .ticks(
                            Math.ceil((this.scale.domain()[1] - this.scale.domain()[0]) / 100) + 1,
                        ),
                )
            ...
            case 'SMART':
                ...
                case 'elli':
                    domainGr.call(
                        this.drawRectDom(
                            dom,
                            (this.scale(dom.end) - this.scale(dom.start)) / 2,
                            this.daHeight / 2,
                        ),
                    );
                    break;
        ...
        return (g) => {
            ...
            if (!this.options.hideDomainLabel) {
                gr.append('text')
                    .attr(
                        'transform',
                        `translate(${(this.scale(dom.end) - this.scale(dom.start)) / 2 - 5}, ${this.daHeight / 2}) rotate(90)`,
                    )
    ...
    getPathCmd(shapeName: string, dom: ProteinDomain): string {
        ...
        const sin18deg = 0.30901699437,
            cos18deg = 0.95105651629,
            sin54deg = 0.80901699437,
            cos54deg = 0.58778525229;
        const sin60deg = 0.86602540378;
        const tan225deg = Math.tan(Math.PI / 8),
            sqrt2 = Math.sqrt(2);
        switch (shapeName) {
        case 'tri_l':
        ...
        return (g) => {
            ...
            if (!this.options.hideDomainLabel) {
                gr.append('text')
                    .attr(
                        'transform',
                        `translate(${this.getTxtX(shapeName, dom)}, ${this.getTxtY(shapeName, dom)}) rotate(90)`,
                    )
                    .attr('text-anchor', 'middle')
                    .attr('fill', dom.shape.textColor)
                    .attr('font-size', '14px')
