...
const config = {
	development: {
		...
		decipher: {
			...
			disease: {
				allowedReferer: process.env.RESTRICT_DECIPHER_DISEASE
					? new RegExp('^(?:https?:\/\/)?(?:www\.)?marrvel.org.*', 'i')
					: new RegExp('.*'),
...
config[env].mongo.url = `mongodb://${config[env].mongo.username}:${
	config[env].mongo.password
}@${config[env].mongo.host}:${config[env].mongo.port}/${config[env].mongo.database}`;
