...
const parseTransvarResultCoordinate = (coord) => {
	return new Promise((resolve, reject) => {
		const M = coord.match(
			/(chr(\d+|X|Y):g.(\d+)((?:A|C|G|T|U)+)?>?((?:(?:[a-z]+)?(?:A|C|G|T|U)+))|\.)\/((?:c\.(\d+(?:[-+_]\d+)?)((?:A|C|G|T|U)+)?)>?((?:[a-z]+)?(?:A|C|G|T|U)+)|\.)\/(p\.([A-Za-z]+)(\d+)([A-Za-z]+|\*)|\.)/,
		);
		if (M == null) {
...
exports.proteinToGenomicLocations = (protein, build) => {
	return new Promise((resolve, reject) => {
		...
		executeTransvar(['panno', '--refversion', build, '-i', protein, '--ensembl', '--refseq'])
			.bind({})
			.then((res) => {
			...
			.filter((candidate) => {
				// Remove this filter if also want to serve mnvs
	...
	return new Promise((resolve, reject) => {
		...
		executeTransvar(['ganno', '--refversion', build, '-i', identifier, '--ensembl', '--refseq'])
			...
			.map((candidate) => {
				...
				return ensembl
					.queryLookupByEnsemblId(candidate.transcriptId)
					.then((res) => {
