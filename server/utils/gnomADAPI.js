...
const variantQuery = fs.readFileSync(path.join(config.root, 'utils/gnomad-variant-query.txt')).toString();
...
const queryByGene = (gene, referenceGenome) => {
	...
	return new Promise((resolve, reject) => {
		...
		got.post('https://gnomad.broadinstitute.org/api/', {
			json: {
				operationName: 'Gene',
				query: geneQuery,
				variables: filter,
			...
			.then((doc) => {
				GnomADGene.updateOne({ entrezId: doc.entrezId }, { $set: doc }, { upsert: true }).catch((err) => {
					console.error(err);
				});
				return resolve(doc);
			})
