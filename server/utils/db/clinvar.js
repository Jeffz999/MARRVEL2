...
exports.getByGeneSymbol = (symbol) => {
	return new Promise((resolve, reject) => {
		Genes.findOne({ taxonId: 9606, symbol: new RegExp(`^${symbol}$`, 'i') }, { chr: 1, hg19Start: 1, hg19Stop: 1 })
			.lean()
			.then((gene) => {
...
const getByGeneEntrezId = async (entrezId) => {
	try {
		const gene = await Genes.findOne({ entrezId: parseInt(entrezId) }, 'chr hg19Start hg19Stop').lean();
