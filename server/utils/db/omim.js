		...
		OMIM.findOne({ mimNumber }, { _id: 0 })
			...
			.then((doc) => {
				if (!doc || !doc.lastUpdate || utils.isOlderThan(doc.lastUpdate, 14)) {
					return utils.omimAPI
						.queryByMimNumber(mimNumber)
						.then((apiRes) => {
							replaceDoc(apiRes);
							return apiRes;
						})
...
const markAllele = (allele, variant) => {
	...
	return new Promise((resolve, reject) => {
		ensembl
			.getGenomicLocationByVariationId(allele.dbSnps)
			.then((mapping) => {
...
const replaceDoc = (doc) => {
	...
	return OMIM.replaceOne({ mimNumber: doc.mimNumber }, doc, { upsert: true }).catch((err) => {
		console.error(err);
	});
};
