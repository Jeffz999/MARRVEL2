				...
				if (!doc) resolve(null);
					...
					for (const key in doc.scores) {
						if (doc.scores[key].score) {
							...
							doc.scores[key].score =
								doc.scores[key].score !== '.' ? parseFloat(doc.scores[key].score) : null;
						}
						if (doc.scores[key].prediction) {
							...
							doc.scores[key].prediction =
								doc.scores[key].prediction !== '.' ? doc.scores[key].prediction : null;
						}
						if (doc.scores[key].rawScore) {
							doc.scores[key].rawScore =
								doc.scores[key].rawScore !== '.' ? parseFloat(doc.scores[key].rawScore) : null;
						}
						if (doc.scores[key].rankscore) {
							doc.scores[key].rankscore =
								doc.scores[key].rankscore !== '.' ? parseFloat(doc.scores[key].rankscore) : null;
						}
					}
					resolve(doc);
				}
