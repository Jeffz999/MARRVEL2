...
router.get('/transvar/protein/:protein', (req, res) => {
	...
	transvar
		.proteinToGenomicLocations(protein, build)
		.then((result) => {
			res.json({
...
router.get('/transvar/forward/gdna/:identifier', (req, res) => {
	...
	if (
		!identifier.match(
			/chr((?:[0-9]+|X|Y|M)):g\.(\d+)(?:_(\d+))?(?:(?:((?:A|C|G|T)+)>((?:A|C|G|T)*))|(?:(delins)((?:A|C|G|T)+))|(?:(del|dup))|(?:(ins)((?:A|C|G|T)*))|(inv))/,
		)
	) {
		res.status(403).end();
	} else {
		transvar
			.forwardAnnotationWithGdna(identifier, build)
			.then((result) => {
				res.json(result);
			})
			.catch((err) => {
