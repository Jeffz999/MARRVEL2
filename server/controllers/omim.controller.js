...
exports.findByMimNumber = (req, res) => {
	...
	db.omim
		.getByMimNumberAndUpdate(mimNumber)
		.then((doc) => {
			res.json(doc);
		})
		.catch((err) => {
...
exports.findByTitle = async (req, res) => {
	...
	let docs = [];
	try {
		docs = await OMIMEntry.find({ $text: { $search: title } }, { score: { $meta: 'textScore' } }).sort({
	...
	for (const doc of docs) {
		...
		if (doc.phenotypeMapList) {
			...
			if (geneIds.length) {
				try {
					genes = await Genes.find(
						{ taxonId: 9606, entrezId: { $in: geneIds } },
